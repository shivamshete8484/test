...
enhancement AccountBaseEnhancement : Account {
  ...
  static function createAccountForContact(contact : Contact) : Account {
    ...
    //ADS: Triggering of Embedded Analytics
    ads.platform.integration.SolutionExecutor.Instance.trigger(new ads.platform.controller.TriggeringPointAccount(ADSTriggeringPointKey.TC_ACCOUNTCREATION, account))
    return account
  }
  ...
  function addAndNumberAccountLocation(location : AccountLocation) {
    ...
    this.LocationAutoNumberSeq.number(location,
        ...
        AccountLocation.Type.TypeInfo.getProperty("LocationNum"))
  }
   ...
   * TargetRelatedAccounts arrays. Each relationship is wrapped in an {@link gw.account.AccountRelationship}
  ...
  function movePoliciesFrom(fromAccount : Account, policyPeriods :PolicyPeriod[]) {
    ...
    createCustomHistoryEventsAndUpdateJobGroups(fromAccount, policiesToMove);
  }

  function createCustomHistoryEventsAndUpdateJobGroups(fromAccount : Account, policiesToMove : Policy[]) {
   ...
   */
  function makeActive() {
    if (not AccountStatus.TF_CANACTIVATE.TypeKeys.contains(this.AccountStatus)) {
  ...
  property get AccountAge_Ext() : int {
    return DateUtil.daysBetween(this.CreateTime, DateUtil.currentDate())
  }

  function mergeWithAccountAndCreateHistoryEvent_Ext(victimAccount: Account){
    try{
      var victimPolicies = Policy.finder.findLocalPoliciesByAccount(victimAccount)
      victimPolicies.each(\ p -> {
        var period = p.LatestBoundPeriod
      ...
      this.mergeWithAccount(victimAccount)
      //history event not added to the surviving account in the base code
      this.createCustomHistoryEvent(CustomHistoryType.TC_ACCT_CHANGED , \ -> DisplayKey.get("Account.History.AccountMerged", victimAccountNumber,
          accountHolderName))
      this.Bundle.commit()
    }
    catch(e: Exception){
